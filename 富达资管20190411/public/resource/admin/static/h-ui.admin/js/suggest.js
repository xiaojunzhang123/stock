(function($) {
	$.suggest = function(input, options) {
		...
		if($.trim($input.val())=='' || $.trim($input.val())=='股票代码/名称（例：600000）') $input.val('股票代码/名称（例：600000）').css('color','#aaa');
		...
		$input.focus(function(){
			if($.trim($(this).val())=='股票代码/名称（例：600000）'){
		...
		function processKey(e) {				
			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					...
					html='<div class="gray ac_result_tip">请输入股票代码/名称</div><ul>'+html+'</ul>'}
					...
					if (html == '') {
						...
						suggest_tip = '<div class="gray ac_result_tip">股票代码/名称：' + items + '，↑↓选择</div>'}
