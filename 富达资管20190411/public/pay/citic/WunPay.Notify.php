 ...
 */
require_once "WunPay.Api.php";
class WunPayNotify extends WunPayNotifyReply
{
	/**
	 * 
	 * 回调入口
	...
	private function notify($callback, &$msg)
	{
		...
		try {
			...
			}else 
			{
				...
				$this->SetKey($key);
				//$result = WunPayResults::Init($xml,$key);
				$result = $obj->GetValues();
			}

			return call_user_func($callback, $result);
			
		} catch (WunPayException $e){
			$msg = $e->errorMessage();
	...
	final private function ReplyNotify($needSign = true)
	{
		...
		$this->SetVersion(WunPayConfig::VERSION);
		
		WunPayApi::replyNotify($this->ToXml());
	}
}