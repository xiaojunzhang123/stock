...
//		    //触发止盈
//		    $("#check-surplus_ul>a").html(price * 5000);
//		    //触发止损
//		    $("#stop-loss_ul>a:eq(0)").html(price * -1000);
//		    $("#stop-loss_ul>a:eq(1)").html(price * -1333);
//		    $("#stop-loss_ul>a:eq(2)").html(price * -1700);
//		    //交易综合费
//		    $("#publicFee>span").html(price * publicFee );
//		    //递延费
...
//		    updateMoneyRate();
		...
		$("#stop-loss_ul > a").on('tap',function(e){
		    ...
		    //总计
		    var total= ly+parseInt($("#publicFee>span").html());
		    $('#total').html(total);
//			//递延条件
        ...
        chartLine: {
            ...
            init: function (selector, json, full) {
                ...
                $.each(json.records, function (i, n) {//第一次   i = 0, n = [1500341400000, 6.45]
                    ...
                    //比较找出最小价格和最大价格
                    ymin = parseFloat(ymin) > parseFloat(n[1]) ? n[1] : ymin;
                    ymax = parseFloat(ymax) < parseFloat(n[1]) ? n[1] : ymax;
