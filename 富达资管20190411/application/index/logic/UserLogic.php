...
class UserLogic
{
    ...
    public function pageUserOrder($userId, $state = 1, $field = "*", $pageSize = 4){
        try{
            ...
            $where['user_id'] = $userId;
            $res = Order::with("belongsToMode")->where($where)->order("update_at DESC")->field($field)->paginate($pageSize);
    ...
    public function getAllOrderIdList($state = 1, $field){
        try{
            $res = Db::table('stock_order')->field($field)->where('state',$state)->select();
            return $res;
        } catch(\Exception $e) {
            return [];
        }
    ...
    // $state 1委托建仓，2抛出，3持仓，4-委托平仓
    public function userOrderById2($userId, $id, $state=null)
    {
        try{
            $where = [];
            $where['order_id'] =  $id;
            $where['user_id'] = $userId;
            $where['state'] = $state;
            $orders =  $res = Db::table('stock_order')->where($where)->select();
            return $orders ? collection($orders)->toArray() : [];
        } catch(\Exception $e) {
            return [];
        }
    ...
    public function pageUserRecords($userId, $type = null, $pageSize = 4){
        try{
            ...
            $res = User::find($userId)->hasManyRecord()->order("create_at DESC")->where($where)->paginate($pageSize);
            return $res ? $res->toArray() : [];
        } catch(\Exception $e) {
            return [];
        }
    ...
    // 上传身份证信息
    public function saveUserCert($userId, $data)
    {
